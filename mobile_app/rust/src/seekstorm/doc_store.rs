...
impl Shard {
    pub(crate) fn get_file_shard(&self, doc_id: usize) -> Result<Vec<u8>, String> {
        let file_path = PathBuf::new(&self.index_path_string)
            .join(FILE_PATH)
    ...
    pub(crate) fn copy_file(&self, source_path: &PathBuf, doc_id: usize) -> io::Result<u64> {
        let dir_path = PathBuf::new(&self.index_path_string).join(FILE_PATH);
        if !dir_path.exists() {
