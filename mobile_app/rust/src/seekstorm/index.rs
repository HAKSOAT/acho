...
/// File type for storing documents: PathBuf, Bytes, None.
#[derive(Clone, PartialEq)]
pub enum FileType {
    /// File path
    PathBuf(Box<PathBuf>),
    /// File bytes
    Bytes(Box<PathBuf>, Box<[u8]>),
    /// No file
    None,
}
...
pub(crate) async fn create_index_root(
    ...
    let file_path = PathBuf::new(index_path_string).join(FILE_PATH);
    if !file_path.exists() {
        if !mute {
    ...
    match File::options()
        ...
        .create(true)
        .truncate(false)
        .open(PathBuf::new(index_path).join(INDEX_FILENAME))
    {
        Ok(index_file) => {
            ...
            if !facets_vec.is_empty()
                && let Ok(file) = File::open(PathBuf::new(index_path).join(FACET_VALUES_FILENAME))
            ...
            if serialize_schema {
                serde_json::to_writer(
                    &File::create(PathBuf::new(index_path).join(SCHEMA_FILENAME)).unwrap(),
                    &schema,
                )
                .unwrap();
                ...
                if !synonyms.is_empty() {
                    serde_json::to_writer(
                        &File::create(PathBuf::new(index_path).join(SYNONYMS_FILENAME)).unwrap(),
                        &synonyms,
                    )
                    .unwrap();
                }
                ...
                serde_json::to_writer(
                    &File::create(PathBuf::new(index_path).join(META_FILENAME)).unwrap(),
                    &index.meta,
                )
                .unwrap();
            }
        ...
        .open(PathBuf::new(index_path).join(INDEX_FILENAME))
    {
        Ok(index_file) => {
            let docstore_file = File::options()
                ...
                .create(true)
                .truncate(false)
                .open(PathBuf::new(index_path).join(DOCSTORE_FILENAME))
                .unwrap();
            ...
            let delete_file = File::options()
                ...
                .create(true)
                .truncate(false)
                .open(PathBuf::new(index_path).join(DELETE_FILENAME))
                .unwrap();
            ...
            let facets_file = File::options()
                ...
                .create(true)
                .truncate(false)
                .open(PathBuf::new(index_path).join(FACET_FILENAME))
                .unwrap();
...
/// * `index_path` - index path.  
/// * `mute` - prevent emitting status messages (e.g. when using pipes for data interprocess communication).  
pub(crate) async fn open_shard(index_path: PathBuf, mute: bool) -> Result<ShardArc, String> {
    if !mute {
...
/// * `index_path` - index path.  
/// * `mute` - prevent emitting status messages (e.g. when using pipes for data interprocess communication).  
pub async fn open_index(index_path: PathBuf, mute: bool) -> Result<IndexArc, String> {
    if !mute {
        println!("opening index ...");
    }
...
impl Shard {
    ...
    async fn clear_shard(&mut self) {
        ...
        let index_path = PathBuf::new(&self.index_path_string);
...
impl Index {
    ...
    pub fn get_synonyms(&self) -> Result<Vec<Synonym>, String> {
        if let Ok(synonym_file) =
            File::open(PathBuf::new(&self.index_path_string).join(SYNONYMS_FILENAME))
        {
    ...
    pub fn set_synonyms(&mut self, synonyms: &Vec<Synonym>) -> Result<usize, String> {
        serde_json::to_writer(
            &File::create(PathBuf::new(&self.index_path_string).join(SYNONYMS_FILENAME)).unwrap(),
            &synonyms,
        )
        .unwrap();
    ...
    async fn close(&self) {
        ...
        if let Some(completion_option) = &self.read().await.completion_option.as_ref() {
            ...
            let completions_path =
                PathBuf::new(&self.read().await.index_path_string).join(COMPLETIONS_FILENAME);
        ...
        if let Some(symspell) = &mut self.read().await.symspell_option.as_ref() {
            let dictionary_path =
                PathBuf::new(&self.read().await.index_path_string).join(DICTIONARY_FILENAME);
